...
our $VERSION = '0.03';
##~ DIGEST : 13cf6d0a2ca9202ef90f9b7328cd8951
use Moo;
use Carp qw/confess/;
use DBI;
with qw/
...
sub _init_sqliteschema {
    ...
    my ( $self, $dbh ) = @_;
    $dbh->do(
"CREATE TABLE grids ( id INTEGER PRIMARY KEY, x INTEGER , y INTEGER, z INTEGER, particle_id INTEGER );"
    ) or die $DBI::errstr;
    $dbh->do(
"	CREATE TABLE particles ( id INTEGER PRIMARY KEY, key BLOB, particle TEXT );"
    )                                                     or die $DBI::errstr;
