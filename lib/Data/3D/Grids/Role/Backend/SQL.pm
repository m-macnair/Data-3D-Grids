...
our $VERSION = '0.05';
##~ DIGEST : 190fc1bb3410480aef7ebe89be935311
use Moo::Role;
use Carp qw/confess/;
OBJECTS: {
	has dbh            => ( is => 'rw' );
	has dbh_autocommit => ( is => 'rw', lazy => 1, builder => '_dbh_autocommit' );
	has sql_abstract   => ( is => 'rw', lazy => 1, builder => '_build_sql_abstract' );
}
STHS: {
	for my $accessor ( qw/set get del get_particle set_particle del_particle get_all_particles / ) {
		has "$accessor\_sth" => ( is => 'rw', builder => "_build_$accessor\_sth", lazy => 1 );
	}
}
ACCESSORS: {
	...
	STHBUILDERS: {
		GRIDS: {
			...
			sub _build_set_sth {
				...
				my ( $self ) = @_;
				my $sth = $self->dbh->prepare( "insert into grids (x,y,z,particle_id) values (?,?,?,?) " )
				  or die $DBI::errstr;
				return $sth;
			...
			sub _build_get_sth {
				...
				my ( $self ) = @_;
				my $sth = $self->dbh->prepare( "select * from grids where x = ? and y = ? and z = ?" )
				  or die $DBI::errstr;
				return $sth;
			...
			sub _build_del_sth {
				...
				my ( $self ) = @_;
				my $sth = $self->dbh->prepare( "delete from grids where x = ? and y = ? and z = ?" )
				  or die $DBI::errstr;
				return $sth;
		...
		PARTICLES: {
			...
			sub _build_set_particle_sth {
				...
				my ( $self ) = @_;
				my $sth = $self->dbh->prepare( "insert into particles (particle,key) values (?,?) " )
				  or die $DBI::errstr;
				return $sth;
			...
			sub _build_get_particle_sth {
				...
				my ( $self ) = @_;
				my $sth = $self->dbh->prepare( "select * from particles where key = ?" )
				  or die $DBI::errstr;
				return $sth;
			...
			sub _build_del_particle_sth {
				...
				my ( $self ) = @_;
				my $sth = $self->dbh->prepare( "delete from particles where key = ?" )
				  or die $DBI::errstr;
				return $sth;
